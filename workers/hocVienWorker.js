...
async function executeTask() {
    try {
        ...
        if (typeof functionToCall !== 'function') {
            throw new Error(`Function ${functionName} is not defined in HocVienModel`);
        }

        // Gọi hàm và truyền các tham số
        const result = await functionToCall(workerData.filter, workerData.page, workerData.pageSize);
