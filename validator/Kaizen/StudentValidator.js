...
class StudentValidator {
    static async arrange(req) {
        ...
        // if (!moment(req.start_date, "YYYY-MM-DD").isValid() || !moment(req.end_date, "YYYY-MM-DD").isValid()) {
        //     code = 1
    ...
    static async transferClasses(req) {
        ...
        if (
            ...
            !after_class ||
            !note ||
            ( !updated_by || !Number.isSafeInteger(updated_by))
        ) {
            code = 1
    ...
    static async create(req) {
        ...
        if (
            !req.first_name ||
            !req.last_name ||
            ( !req.contact_id || !Number.isSafeInteger(req.contact_id)) ||
            !req.dob ||
            !req.native_place ||
            ( !req.deal_id || !Number.isSafeInteger(req.deal_id)) ||
            !req.deal_opening_date ||
            !req.deal_program_bitrix_id ||
            ( !req.created_by || !Number.isSafeInteger(req.created_by)) ||
        ...
        if (!moment(req.dob, "YYYY-MM-DD").isValid() || !moment(req.deal_opening_date, "YYYY-MM-DD").isValid()) {
            code = 1
            return {code: code, message: "Date not valid"}
        }

        // const phone = req.^\d{10}$
        ...
        if (
            schools_id && 
            !Number.isSafeInteger(schools_id)
        ) {
            code = 1
        ...
        if (
            date_graduation && 
            !moment(date_graduation, "YYYY-MM-DD").isValid()
        ) {
            code = 1
    ...
    static async update(req)
    {
        ...
        if (
            ...
            !req.deal_opening_date ||
            !req.deal_program_bitrix_id ||
            ( !req.updated_by || !Number.isSafeInteger(req.updated_by)) ||
