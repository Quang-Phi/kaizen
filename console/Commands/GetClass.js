...
class GetClass {
    ...
    static async handle(params) 
    {
        ...
        console.info('%cSignature: %c%s', 'color: blue; font-weight: bold;', 'color: #444;', GetClass.signature);
        console.info('%cDescription: %c%s', 'color: blue; font-weight: bold;', 'color: #444;', GetClass.description);
        ...
        */
        ...
        if (data) {
            ...
            for (const value of data) {
                ...
                // classes.status = value.classStatus == "Open" ? 1 : 2;
                ...
                let ClassesRoomData = {};
                const rsClassesRoom = Branches.find(({ sharepoint_id }) => sharepoint_id === value.branchID);
                ClassesRoomData.branch_code = rsClassesRoom.code;
                ClassesRoomData.name = value.roomName;
            
                if (ClassesRoomData.name) {
                    const ClassesRoom = await ClassesRoomModel.createOnUpdate(ClassesRoomData, {
                ...
                if (value.teachersJoinClasses && value.teachersJoinClasses.length > 0) {
                    const teachersJoinClasses = value.teachersJoinClasses.filter(({ id, role, isClosed, handOverDate }) => {
                        return id > 0 && role == "Giáo viên chủ nhiệm" && isClosed == false && handOverDate == null;
                    });
                    // Tiếp tục xử lý `teachersJoinClasses` nếu cần
                    // console.log(teachersJoinClasses, 'teachersJoinClasses');
                    Logs.logText('req-commands-get-class', JSON.stringify(teachersJoinClasses));
                    if (teachersJoinClasses[0]?.id) {
                        teacher_code = await TeachersModel.find(['code'], {sharepoint_id: teachersJoinClasses[0].teacherID});
                    }
                }
                ...
                await RegisterClassModel.createOnUpdate({
                    ...
                    opening_date: value.openingDate ? moment(value.openingDate).format("YYYY-MM-DD") : null,
                    start_date: value.startDate ? moment(value.startDate).format("YYYY-MM-DD") : null,
                    end_date: value.endDate ? moment(value.endDate).format("YYYY-MM-DD") : null,
