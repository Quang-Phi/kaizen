...
const { PtnLessonLessonGroupModel } = require('../../models/Kaizen/PTN/PtnLessonLessonGroupModel');
const { PtnLessonGroupModel } = require('../../models/Kaizen/PTN/PtnLessonGroupModel');
...
class CoursesLessonLessonGroupServices {
     ...
     static async getList(params) 
     {
          ...
          if (rs.length) {
               ...
               ptn_lesson = new Map(
                    ptn_lesson.map(item => [item.code, {...item}])
               );
     
               rs = rs.map((item) => {
     
                    const ptn_lesson_value = ptn_lesson.get(item.ptn_lesson_code)?.value || null;
     ...
     static async getLessonGroupByPointTypes(params)
     {
          let rs = await RawQueryModel.getRaw(
               `
                    ...
                    ${params.ptn_courses_point_types_code ? 'WHERE ptn_courses_point_types_code =?' : ''}
                    GROUP BY ptn_lesson_group_code
               `,
               params.ptn_courses_point_types_code ? [params.ptn_courses_point_types_code] : []
          );
          ...
          if (rs.length) {
               ...
               rs = rs.map(item => {
                    ...
                    const lesson_group_name = lesson_group_code.get(item.ptn_lesson_group_code)?.name || null;
