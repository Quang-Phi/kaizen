class Model {
    ...
    static async create(data) {
        ...
        return {
            ...
            placeholders: `(${Object.keys(filteredData).map(() => '?').join(', ')})`,
    ...
    static async updated(data, where = []) {
        const [filteredData, filteredWhere] = await Promise.all([this.filteredData(data), this.filteredData(where)]);

        return {
            columns: Object.keys(filteredData).join(', '),
            placeholders: Object.keys(filteredData).map(key => `${key} = ?`).join(', '),
            // values: Object.values(filteredData).push(Object.values(filteredWhere))
            values: [...Object.values(filteredData), ...Object.values(filteredWhere)],
            wheres: Object.keys(filteredWhere).map(key => `${key} = ?`).join(' AND '),
        }
    }
    ...
    static async createMultiple(data) {
        ...
        return {
            ...
            placeholders: filteredData.map(() => `(${Object.keys(filteredData[0]).map(() => '?').join(', ')})`).join(', '),
    ...
    static async filteredData(data)
    {
        return Object.keys(data).reduce((acc, key) => {
            if (this.fillable.includes(key) && typeof data[key] !== 'undefined' || (this.primaryKey === key && typeof data[this.primaryKey] !== 'undefined')) {
                acc[key] = data[key];
            }
    ...
    static async filteredDataMultiple(data) {
        const filteredData = data.map(item => {
            // Tạo một đối tượng mới chỉ chứa các thuộc tính có trong `fillable`
            return Object.keys(item).reduce((acc, key) => {
                if (this.fillable.includes(key) && typeof data[key] !== 'undefined' || (this.primaryKey === key && typeof data[this.primaryKey] !== 'undefined')) {
                // if (this.fillable.includes(key) || this.primaryKey == key) {
                    acc[key] = item[key];
                }
                return acc;
            }, {});
